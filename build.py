...
from utils import gout, current_path, containers_path
from os import system, environ
from dotenv import load_dotenv
load_dotenv(current_path.joinpath(".env"))
environ["TERM"] = "xterm"
...
class Container:
    ...
    def __init__(self, folder):
        self.path = containers_path.joinpath(folder)
        self.path_content = self.path.joinpath("photos")
        self.folders = list(filter(lambda fn: fn.is_dir(), self.path_content.iterdir()))

        def getdata(self):
            months = ("0{}".format(m) for m in range(1, 13))
            for folder in self.folders:
                for month in list(filter(lambda m: m.name in months, folder.iterdir())):
